...
export const AuditResults = ({ result, onReset, lead = null }: AuditResultsProps) => {
  ...
  const { resolved, canResolve, toggle } = useResolvedIssues(result.reportId);
        ...
        {activeSection === 'performance' && (
          <div className="space-y-4">
            ...
            {result.issues.filter(i => i.category === 'Performance').map((issue, index) => (
              <IssueCard key={issue.id} issue={issue} index={index} isResolved={resolved.has(issue.id)} canResolve={canResolve} onToggleResolved={toggle} />
            ))}
